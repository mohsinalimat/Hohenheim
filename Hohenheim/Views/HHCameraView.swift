...
@objc protocol HHCameraViewDelegate: class {
    func cameraShotFinished(_ image: UIImage)
}

final class HHCameraView: UIView, UIGestureRecognizerDelegate {
...
//    var croppedAspectRatioConstraint: NSLayoutConstraint?
    ...
    static func instance() -> HHCameraView {
        
        return UINib(nibName: "HHCameraView", bundle: Bundle(for: self.classForCoder())).instantiate(withOwner: self, options: nil)[0] as! HHCameraView
    }
    ...
    func initialize() {
        ...
        flashOnImage = HohenheimConfiguration.flashOnImage ?? UIImage(named: "ic_flash_on", in: bundle, compatibleWith: nil)
        ...
        let shotImage = HohenheimConfiguration.shotImage ?? UIImage(named: "ic_shutter", in: bundle, compatibleWith: nil)
        ...
        if let _device = device, let _videoInput = try? AVCaptureDeviceInput(device: _device) {
            ...
            let tapRecognizer      = UITapGestureRecognizer(target: self, action:#selector(HHCameraView.focus(_:)))
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(HHCameraView.willEnterForegroundNotification(_:)), name: NSNotification.Name.UIApplicationWillEnterForeground, object: nil)
    }
        ...
        DispatchQueue.global(qos: .default).async(execute: { () -> Void in
            ...
            imageOutput.captureStillImageAsynchronously(from: videoConnection!) { (buffer, error) -> Void in
                ...
                DispatchQueue.main.async {
                    ...
                    if HohenheimConfiguration.shouldAutoSavesImage {
...
fileprivate extension HHCameraView {
    ...
    @objc func focus(_ recognizer: UITapGestureRecognizer) {
        ...
        focusView.layer.borderColor = HohenheimConfiguration.baseTintColor.cgColor
